...
const modelUrl = "https://huggingface.co/lmstudio-community/Llama-3.2-1B-Instruct-GGUF/resolve/main/Llama-3.2-1B-Instruct-Q4_K_M.gguf";
const modelPath = path.join(process.cwd(), "Llama-3.2-1B-Instruct-Q4_K_M.gguf");

// ==========================================
// 1. LLAMA GGUF MODELİNİN İNDİRİLMESİ VE YÜKLENMESİ
// ==========================================
...
async function initLlama() {
  try {
    ...
    if (!fs.existsSync(modelPath)) {
      console.error(`\nHata: Llama GGUF dosyası ana dizinde bulunamadı!`);
      process.exit(1);
    }
    ...
    console.log("🦙 Llama-3.2-1B GGUF hazır!");
  } catch (err) {
    console.error("Llama başlatılırken kritik hata oluştu:", err);
    process.exit(1);
  }
}
...
// ==========================================
// 2. İNFERANS VE DÜŞÜNME MODLARI AKIŞI
// ==========================================
...
async function processGoobResponse(trimmedText, mode = "short") {
  try {
    ...
    if (!llamaSession) return "Model henüz hazır değil.";
...
// ==========================================
// 4. SUNUCU BAŞLATMA (RENDER UYUMLU)
// ==========================================

async function main() {
  try {
    // Önce model dosyasının varlığını kontrol et ve gerekiyorsa indir
    await downloadModelIfNeeded();

    // Ardından Llama modelini başlat
    await initLlama();

    const server = app.listen(PORT, () => {
      console.log(`\n======================================================`);
      console.log(`🌍 WEB SİTESİ BURADA AKTİF: http://localhost:${PORT}`);
      console.log(`🦙 Sunucu Render üzerinde başarıyla başlatıldı.`);
      console.log(`======================================================\n`);
    });
